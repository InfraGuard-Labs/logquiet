...
Easiest methods first; see [docs/RELEASE_PROCESS.md](docs/RELEASE_PROCESS.md)
for how each of these is built and what's still pending.
...
Downloads the right binary for your platform, verifies its SHA256 against
the release's `SHA256SUMS.txt`, and installs to `~/.local/bin` (no `sudo`
needed). Review it before piping it to a shell you didn't write:
`curl -fsSLo install.sh .../install.sh && less install.sh && sh install.sh`.
Supports `LOGQUIET_VERSION`, `LOGQUIET_INSTALL_DIR`, and `LOGQUIET_BASE_URL`
overrides - see the script's own header comment.

> **Known issue:** `v0.1.0` is currently marked as a GitHub *pre-release*,
> which makes the script's default "latest version" auto-detection 404
> (GitHub's `/releases/latest` API deliberately excludes pre-releases).
> Until that's corrected, pin the version explicitly:
> `LOGQUIET_VERSION=v0.1.0 sh install.sh` (or pipe from curl with the env
> var set the same way). This has been verified end-to-end against the
> live release with the version pinned.
...
which directory you're in), and put it on your `PATH`.

### Debian / Ubuntu

```bash
sudo dpkg -i logquiet_<version>_amd64.deb   # or _arm64.deb
```

`.deb` packages (install to `/usr/bin/logquiet`; no service, no cron job,
no created user, no network listener - see
[SECURITY.md](SECURITY.md)) are generated by
[packaging/nfpm/nfpm.yaml](packaging/nfpm/nfpm.yaml) and validated with
`dpkg-deb --info`/`--contents` plus a real install/run/uninstall cycle in
a disposable container. They are not yet attached to a published GitHub
Release - that starts with the next tagged release (see "Recommended next
version" below); build one locally today with `nfpm` (`packaging/nfpm/`)
against a binary from `scripts/build-release.sh`.

### Fedora / RHEL (RPM)

```bash
sudo rpm -i logquiet-<version>-1.x86_64.rpm   # or .aarch64.rpm
```

Same package definition and validation as the `.deb` above, via the same
[nfpm](https://nfpm.goreleaser.com) config - not yet attached to a
published release either.

### Homebrew (not yet published)

```bash
brew install InfraGuard-Labs/tap/logquiet
```

This does **not work yet** - `InfraGuard-Labs/homebrew-tap` does not
exist as a repository. The formula is prepared and generated correctly
(`.goreleaser.yaml`, validated with a Ruby syntax check) at
[packaging/homebrew/logquiet.rb.tmpl](packaging/homebrew/logquiet.rb.tmpl);
publishing it requires creating that tap repository, a decision outside
what this process makes on its own.

### Scoop (not yet published)

```bash
scoop bucket add infraguard https://github.com/InfraGuard-Labs/scoop-bucket
scoop install logquiet
```

Also **not available yet** - `InfraGuard-Labs/scoop-bucket` does not
exist. The manifest is prepared at
[packaging/scoop/logquiet.json.tmpl](packaging/scoop/logquiet.json.tmpl)
(valid JSON, verified) and ready to publish once that repository exists.
...
## Using LogQuiet in the real world?

If LogQuiet is useful in your environment, real-world feedback is
welcome - what worked, what didn't, anything suppressed that shouldn't
have been. Please do not share confidential logs or credentials; describe
symptoms instead, or attach a voluntarily generated, content-free
`--impact-report` (see [docs/IMPACT_REPORT.md](docs/IMPACT_REPORT.md)).
Use the
[real-world feedback issue template](.github/ISSUE_TEMPLATE/real-world-feedback.yml)
or see [docs/ADOPTER_FEEDBACK_FORM.md](docs/ADOPTER_FEEDBACK_FORM.md) for
the full (entirely optional) field list. Nothing here is required, and no
usage data is ever collected automatically - see
